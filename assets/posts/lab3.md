...
Safety controller: 
SafetyController subscribes to the /scan and /vesc/ackermann_cmd_mux/output (changed) topics, and publishes to /vesc/ackermann_cmd_mux/input/safety. It slices the laser scan data into the range -pi/3 and pi/3, then compares the minimum distance from that range to the threshhold safety distance. If the navigation program is trying to drive the car forward (as determined from suscribing to ____) and the car is within .7 m or .35 m of an obstacle, the car publishes a command of either a reduced or 0 velocity, respectively. 
...
can't teleop, wrote test script continuously publishing commands to move forward
